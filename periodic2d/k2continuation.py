...
class lcsolver(object):
    ...
    def solve(self, K2):
        print(GREEN % ("Solving for K2 = %s" % K2))
        self.K2.assign(K2)
        ...
        try:
            ...
            print("Warning: the solver did not converge at K2=%s" % K2)
            nonlinear_its = 1
            linear_its = 0
...
if __name__ == "__main__":
    ...
    start = 0.2 #should always keep K2 positive
    end = 8.0
    step = 0.1
    K2s = list(numpy.arange(start,end,step)) +[end]
    results = []
    for K2 in K2s:
        (n, info_dict) = solver.solve(K2)
        upvdf.write(n, time=K2)
    ...
    plt.title("%s" % pdfname)
    plt.ylim((0,6))
    plt.plot(K2s, [a/b for a, b in zip(linear_iter, nonlinear_iter)], "b", label="Aver. KSP iter.")
    plt.legend()
    plt.savefig("output/KSP_K2_" + pdfname + ".pdf")
